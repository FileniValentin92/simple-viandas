...
function PackBox({ cant, precio }) {
  ...
  return (
    ...
    >
      ...
      <p style={{ fontSize: '10px', color: hovered ? 'var(--gold)' : 'rgba(14,14,12,0.6)', fontFamily: 'Jost, sans-serif', fontWeight: '300', transition: 'all 0.25s ease' }}>
        desde {precio}
      </p>
    </div>
  )
  ...
  return (
    <section style={{ background: 'var(--black)', padding: 'clamp(32px, 5vw, 56px) clamp(20px, 5vw, 80px)' }}>
      <div style={{ border: '1px solid var(--gold)', background: 'var(--cream)', padding: 'clamp(40px, 5vw, 64px) clamp(24px, 4vw, 56px)', maxWidth: '900px', margin: '0 auto', textAlign: 'center' }}>
        <p style={{ fontSize: '9px', letterSpacing: '4px', textTransform: 'uppercase', color: 'rgba(14,14,12,0.4)', fontFamily: 'Jost, sans-serif', fontWeight: '300', marginBottom: '12px' }}>
        ...
        <p style={{ fontSize: '14px', color: 'rgba(14,14,12,0.6)', fontFamily: 'Jost, sans-serif', fontWeight: '300', lineHeight: '1.7', maxWidth: '480px', margin: '0 auto 40px' }}>
