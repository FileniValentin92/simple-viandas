...
export default function LoginPage() {
  ...
  const handleRegistro = async () => {
    ...
    if (error) { setError('Error al crear la cuenta: ' + error.message); setCargando(false); return }
        ...
        <div style={{ width: '100%', maxWidth: '440px' }}>
          ...
          <div style={{ display: 'flex', marginBottom: '40px', borderBottom: '1px solid rgba(247,243,236,0.15)' }}>
            {[{ id: 'login', label: 'Iniciar sesión' }, { id: 'registro', label: 'Crear cuenta' }].map(tab => (
              <button key={tab.id} onClick={() => { setModo(tab.id); setError(''); setExito('') }} style={{
          ...
          <p style={{ fontSize: '12px', color: 'rgba(247,243,236,0.4)', textAlign: 'center', fontWeight: '300' }}>
            ...
            <button onClick={() => setModo(modo === 'login' ? 'registro' : 'login')} style={{ background: 'none', border: 'none', color: 'var(--gold)', cursor: 'pointer', fontSize: '12px', fontFamily: 'Jost, sans-serif' }}>
